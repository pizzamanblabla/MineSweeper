...
-(void) setIsShown: (BOOL) isShown {
    ...
    if (isShown) {
        ...
        } else if (![self.backgroundColor isEqual:[MineSweeperPaletteFactory backgroundCellColorOfBombExploded:PALETTE]]) {
...
-(SystemSoundID) tapSound {
    if (!_tapSound) {
        NSURL *toneURLRef = [[NSBundle mainBundle] URLForResource:@"tapSound" withExtension:@"wav"];
...
-(SystemSoundID) flagSound{
    if (!_flagSound) {
        NSURL *toneURLRef = [[NSBundle mainBundle] URLForResource:@"flagSound" withExtension: @"wav"];
